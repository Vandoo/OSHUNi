...
//--------------------------------------------------------------
    ...
    Current_xyz::calculate_J(Stat& Yin) {                           //WHY- same as Vlasovmax: Current_Jxyz, but save Jxyz in JN.jay_xyz
//--------------------------------------------------------------
...
//--------------------------------------------------------------
       : t(static_cast<double>(tout_start) *
         ...
         f00(Y.SH(0,0)),
         f10(f00), f11(f00),                                    //WHY? in advance(), f10,..., f22 are redifined again
...
//--------------------------------------------------------------
        ...
        FindDE();                           //  Reset DE                                                    //WHY- actually DE !=0, ~epsilon

// - - - - - - - - - - - - - - - - - - - - - -
        while ( (zeros_in_det > 0) && ( execution_attempt < 4) ) {  // Execute this loop at most twice      //WHY? twice? but <4
...
// - - - - - - - - - - - - - - - - - - - - - -
            ...
            J0.calculate_J(Y);                                                                              //WHY? E=0? it seems E=En
            Y.SH(0,0) = f00;                                                                                //WHY- after getting J0, recover Y..
...
// - - - - - - - - - - - - - - - - - - - - - -
            ...
            if ( !(if_implicit1D) ) {                                                                       //WHY- implicit1D == false, always do this
...
//  Reset DE
//--------------------------------------------------------------
        double Eps(16.0*numeric_limits<double>::epsilon());             //WHY- very small number
