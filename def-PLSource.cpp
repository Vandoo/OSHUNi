...
//--------------------------------------------------------------
       : t1(tout_start*Inputdata::IN().cont().dt_out), 
         Y00(Yin.SH(0,0)),
         nmin(0.00001), 
         acoeff(2.08050325*1.0e+12/(Inputdata::IN().inp().xglob.dx()*                                   //WHY- P30
                                    Inputdata::IN().list().density_np*
                                    Inputdata::IN().list().lambda_0)),
         linear_Ivst(Inputdata::IN().list().linear_Ivst),
         T0(1000.0 * sqrt(1.0e-14*Inputdata::IN().list().lambda_0*Inputdata::IN().list().lambda_0)),    //WHY? 1000.0?
         polynomial_Ivst(Inputdata::IN().list().polynomial_Ivst),
         pr(Inputdata::IN().inp().pr), 
         gamma(Inputdata::IN().inp().pr),                                                               //gamma for relativistic
         fp4(pr),
...
//**************************************************************
//**************************************************************
// External electric field and magnetic field are introduced
//**************************************************************
//**************************************************************
//--------------------------------------------------------------
    External_EBfield::External_EBfield(Stat& Yin, int tout_start)
//--------------------------------------------------------------
//  Constructor
//--------------------------------------------------------------
       : t1(tout_start*Inputdata::IN().cont().dt_out),
         omega_0(3.0e+10*2.0*M_PI/(1.0e-4*Inputdata::IN().list().lambda_0)),
         omega_p(5.64 * 1.0e+4*sqrt(Inputdata::IN().list().density_np)),
         w0overwp(2.0*M_PI/(Inputdata::IN().list().ymax-Inputdata::IN().list().ymin)*(Inputdata::IN().list().numy_glob-1)/Inputdata::IN().list().numy_glob), 
         polarization(Inputdata::IN().list().polarization_direction), 
         LaserFields(Yin.EMF()),
//       -----
         ex_electric(Inputdata::IN().list().ex_electric),
         ex_magnetic(Inputdata::IN().list().ex_magnetic),
//       -----
         vos(Inputdata::IN().list().lambda_0 * sqrt(7.3e-19*Inputdata::IN().list().I_0)),
         IL_xprofile(Inputdata::IN().inp().x.dim()),
         IL_yprofile(Inputdata::IN().inp().y.dim()),
         xaxis(Inputdata::IN().inp().x),
         yaxis(Inputdata::IN().inp().y),
         rise_time(Inputdata::IN().list().rise_time),
         fall_time(Inputdata::IN().list().fall_time),
         flat_time(Inputdata::IN().list().flat_time){
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
//--------------------------------------------------------------
    int External_EBfield:: force()  const {return ex_electric || ex_magnetic;} 
    int External_EBfield:: POL()  const {return polarization;} 
//--------------------------------------------------------------

//--------------------------------------------------------------
    void External_EBfield::EB_field(const double& tnew){ 
//--------------------------------------------------------------
        double dt = tnew -t1;
        ...
        switch (POL()) {
            case 1: {
               for (size_t i(0); i < Inputdata::IN().inp().x.dim(); ++i){
                    for (size_t j(0); j < Inputdata::IN().inp().y.dim(); ++j){
                        //LaserFields.Ex()(i,j) = vos ;//* IL_xprofile[i] * IL_yprofile[j] 
                                //* Envelope_Value * Carrier_Signal(tnew-xaxis(i));
						LaserFields.Ex()(i,j) = 1.0e-8;//3.64e-6 tested this 
                    }
                }
                break;
            }
            case 2: {
                for (size_t i(0); i < Inputdata::IN().inp().x.dim(); ++i){
                    for (size_t j(0); j < Inputdata::IN().inp().y.dim(); ++j){
						LaserFields.By()(i,j) = 1.e-02;
						if(abs(tnew)< 0.001){
							LaserFields.Bx()(i,j) = 1.059263468e-06*Carrier_Signal(yaxis(j));
							LaserFields.Bz()(i,j) = -1.059263468e-06*Carrier_Signal_cos(yaxis(j));
						}
						//LaserFields.Bx()(i,j) = -6.e-06*Carrier_Signal(yaxis(j)+1.0e-2*tnew);
						/*if ( ex_magnetic ) {
							LaserFields.By()(i,j) = 1.0e-2 ;
							//if(abs(tnew) < 2.99){
								//LaserFields.Bz()(i,j) = 6.e-06*Carrier_Signal_cos(yaxis(j)+1.0e-2*tnew);
								//LaserFields.Bx()(i,j) = -6.e-06*Carrier_Signal(yaxis(j)+1.0e-2*tnew);
							//}

						}*/
						//if ( ex_electric ) {
						//	LaserFields.Ex()(i,j) = 1.0e-6;//3.64e-6 tested this 
						//}
                    }
                }
                break;
            ...
            case 3: {
                for (size_t i(0); i < Inputdata::IN().inp().x.dim(); ++i){
                    for (size_t j(0); j < Inputdata::IN().inp().y.dim(); ++j){
						//if ( ex_magnetic ) {
							LaserFields.Bz()(i,j) = 1.0e-5;							//WHY- need to change E(np,cwp)
						//}
						/*if ( ex_electric ) {
							LaserFields.Ey()(i,j) = 1.0e-8;//3.64e-6 tested this 
						}*/
						//LaserFields.Ez()(i,j) = vos * IL_xprofile[i] * IL_yprofile[j] 
                        //      * Envelope_Value * Carrier_Signal(tnew-xaxis(i));
                    }
...
//--------------------------------------------------------------
    double External_EBfield::Carrier_Signal(const double& phase){ 
//--------------------------------------------------------------
//  The carrier signal of the laser
//--------------------------------------------------------------
        return sin(w0overwp*phase);
    }
    double External_EBfield::Carrier_Signal_cos(const double& phase){ 
        return cos(w0overwp*phase);
    }
//--------------------------------------------------------------
