...
//--------------------------------------------------------------
		...
		//for(int ix(Nx-4); ix < Nx; ++ix){
			//cout<<ix<<": "<<Y.SH(1,0)(2,ix,0)<<endl;
		//	cout<<ix<<" Pl: "<<Y.EMF().Ex()(ix,3)<<endl;
		//}
...
//--------------------------------------------------------------
    void Node_Communications::contin_bound_Xleft(Stat& Y) {
        int Width;
        size_t Nx(Y.SH(0,0).numx());
        ...
        for(int l(0); l < Y.DF().l0(); ++l){
            for(int m(0); m < ((Y.DF().m0() < l)? Y.DF().m0():l)+1; ++m){
                ...
                for (int c(0); c < Width-1; ++c) 
                    copy(Y.SH(l,m).x0(Width-1), Y.SH(l,m).x0(Width-1).end(), Y.SH(l,m).x0(c));
            }
		}
        ...
        for(int i(0); i < Y.EMF().dim(); ++i){
           ...
           for (int c(0); c < Width; ++c) 
              copy(Y.FLD(i).x0(Width-c-1), Y.FLD(i).x0(Width-c-1).end(), Y.FLD(i).x0(c));
        }

	}
//--------------------------------------------------------------
    void Node_Communications::smooth_bound_Xleft(Stat& Y) {
        using Inputdata::IN; 
        size_t Nx(Y.SH(0,0).numx());
		size_t yid;

	    // SHarmonics	
        for (size_t l(0); l<IN().inp().l0+1; ++l){
            for (size_t m=0; m<((IN().inp().m0<l)?IN().inp().m0:l)+1; ++m){
		        SHarmonic& fs(Y.SH(l,m));
                //
                for (int iy(0); iy < szy; ++iy){
					yid = iy+Nbc-1;
                    for (size_t ip(0); ip < fs.nump(); ++ip){
                        // left boundary
						fs(ip,Nbc-1,yid) = 2.0*fs(ip,Nbc,yid) - fs(ip,Nbc+1,yid);
                    }
                }
        ...
        for (int i(0); i < Y.EMF().dim(); ++i){
            ...
            for (int iy(0); iy < szy; ++iy){
			    yid = iy+Nbc-1;
                // left boundary
			    VD(Nbc-1,yid) = 2.0*VD(Nbc,yid) - VD(Nbc+1,yid);
           }
        }
...
//--------------------------------------------------------------
//--------------------------------------------------------------
    void Node_Communications::contin_bound_Xright(Stat& Y) {
        int Width;
        size_t Nx(Y.SH(0,0).numx());
        ...
        for(int l(0); l < Y.DF().l0(); ++l){
            for(int m(0); m < ((Y.DF().m0() < l)? Y.DF().m0():l)+1; ++m){
                // right boundary
                for (int c(0); c < Width-1; ++c) 
                    copy(Y.SH(l,m).x0(Nx-Width), Y.SH(l,m).x0(Nx-Width).end(), Y.SH(l,m).x0(Nx-c-1));
            }
		}
        ...
        for(int i(0); i < Y.EMF().dim(); ++i){
           // right boundary
           for (int c(0); c < Width; ++c) 
               copy(Y.FLD(i).x0(Nx-Width), Y.FLD(i).x0(Nx-Width).end(), Y.FLD(i).x0(Nx-c-1));
        }

	}
//--------------------------------------------------------------
    void Node_Communications::smooth_bound_Xright(Stat& Y) {
        using Inputdata::IN; 
        size_t Nx(Y.SH(0,0).numx());
        size_t Nl = Nx- Nbc;
		size_t yid;
        ...
        for (size_t l(0); l<IN().inp().l0+1; ++l){
            for (size_t m=0; m<((IN().inp().m0<l)?IN().inp().m0:l)+1; ++m){
                ...
                for (int iy(0); iy < szy; ++iy){
                    ...
                    for (size_t ip(0); ip < fs.nump(); ++ip){
                        // right boundary
						fs(ip,Nl,yid) = 2.0*fs(ip,Nl-1,yid) - fs(ip,Nl-2,yid);
                    }
                }
        ...
        for (int i(0); i < Y.EMF().dim(); ++i){
            ...
            for (int iy(0); iy < szy; ++iy){
	 		    yid = iy+Nbc-1;
                // right boundary
  			    VD(Nl,yid) = 2.0*VD(Nl-1,yid) - VD(Nl-2,yid);
            }
        }
...
//
//--------------------------------------------------------------
//  copy boundary in the x direction for 1 node
//--------------------------------------------------------------
    void Node_Communications::sameNode_contin_X(Stat& Y) {
        int Width;
        size_t Nx(Y.SH(0,0).numx());
...
//--------------------------------------------------------------
    void Node_Communications::sameNode_smooth_X(Stat& Y) {
        using Inputdata::IN; 
        size_t Nx(Y.SH(0,0).numx());
        size_t Nl = Nx- Nbc;
		size_t yid;
        complex<double> At, Bt;
...
//--------------------------------------------------------------
        ...
        if (NODES() > 1) { 
		 ...
		 //if (RANK() == 0){
		//	for(int ix(szx-2); ix < szx; ++ix){
				//cout<<ix<<": "<<Y.SH(1,0)(2,ix,0)<<endl;
		//		cout<<ix<<" Me: "<<MExglob(ix,1)<<endl;
		//	}
		// }
...
//--------------------------------------------------------------
        ...
        for (int iy(0); iy < szy; ++iy){
            for (int ix(0); ix < szx; ++ix){
                ...
                Vy(ix,iy) *= 0.5;
				//Vz
				if (Inputdata::IN().list().o_Vz) {
					Vz(ix,iy)  = F11(0,ix+Nbc,iy+Nbc).imag() * pow(pr(0),3)*invgamma(0);
					Vz(ix,iy) += F11(F11.nump()-1,ix+Nbc,iy+Nbc).imag() * 
                                 pow(pr(pr.dim()-1),3)*invgamma(pr.dim()-1);
					Vz(ix,iy) *= 0.5;
				}
                ...
                for (size_t ip(1); ip < F11.nump()-1; ++ip){ 
					...
					//Vz
					if (Inputdata::IN().list().o_Vz)
						Vz(ix,iy)+= pow(pr(ip),3)*invgamma(ip)*F11(ip,ix+Nbc,iy+Nbc).imag();
                }
		...
		//Vz
        if (Inputdata::IN().list().o_Vz) {
            string fname("OUTPUT/MOM/Vz/Vz");
            parallel_mom(step, Vz, fname);
        }
        // --------------------------------------------------------
...
//          Convert the Mean Square Velocity to Temperature in eV
            //Vsq *= 170880.776; 
			//--- e-i
            Vsq *= 3.131736e+08;													//WHY- 511keV/3, P2, thermal momentum
            string fname("OUTPUT/MOM/T_eV/T");
            parallel_mom(step, Vsq, fname);
            //Vsq *= 1.0/170880.776;
			//--- e-i
            Vsq *= 1.0/3.131736e+08;
        }
...
//          Convert the Mean Square Velocity to Pressure in Mbar
			...
			//--- e-i
            Pre *= 2.73785179e-19 *1836.125321* Inputdata::IN().list().density_np;	//WHY? 2.73e-19
            for (int iy(0); iy < szy; ++iy){
                for (int ix(0); ix < szx; ++ix){
...
//      ND
        ...
        for (int iy(0); iy < szy; ++iy){
			...
			//--- e-i
                ND(ix,iy) = 1.72e+9*sqrt(pow(Vsq(ix,iy)*3.131736e+08,3) /			//WHY? 1.72e9
...
//--------------------------------------------------------------
    void Parallel_Output::parallel_Jamp(size_t step, Stat& Y) {
//--------------------------------------------------------------
//  Calculation of the output matrices related to the current
//  now using J = curlxB, but will be changed later
//--------------------------------------------------------------
        using Inputdata::IN; 
        Field2D Ji(IN().inp().x.dim(),IN().inp().y.dim());
        Matrix2D<float> Jout(szx,szy);  

        //------------------------------------Jx = dBz/dy      
        Field2D tmpJi(Y.EMF().Bz());
        if (Inputdata::IN().list().o_Jx) {
            tmpJi *= idy;
            Ji     = tmpJi.Dy();
            for (int iy(0); iy < szy; ++iy){
                for (int ix(0); ix < szx; ++ix){
                     Jout(ix,iy)  = Ji(ix+Nbc,iy+Nbc).real();
                }
            }
            // Decide if you want to output Jx
            string fname("OUTPUT/FLD/JX/Jx");
            parallel_mom(step, Jout, fname);
        }
        //------------------------------------Jy = -dBz/dx 
        if (Inputdata::IN().list().o_Jy) {
            tmpJi    = Y.EMF().Bz();
            tmpJi   *= (-1.0)*idx;
            Ji       = tmpJi.Dx();
            for (int iy(0); iy < szy; ++iy){
                for (int ix(0); ix < szx; ++ix){
                     Jout(ix,iy)  = Ji(ix,iy).real();
                }
            }
            // Decide if you want to output Jy
            string fname("OUTPUT/FLD/JY/Jy");
            parallel_mom(step, Jout, fname);
        }
        //------------------------------------Jz =
        if (Inputdata::IN().list().o_Jz) {
            //-dBx/dy
            tmpJi    = Y.EMF().Bx();
            tmpJi   *= (-1.0) * idy;
            Ji       = tmpJi.Dy();
            ...
            // Decide if you want to output Jz
            string fname("OUTPUT/FLD/JZ/Jz");
            parallel_mom(step, Jout, fname);
        }

    }
//--------------------------------------------------------------
    void Parallel_Output::parallel_Jv(size_t step, Stat& Y) {		//WHY- using Vx/y/z routine
        SHarmonic&  F0(Y.SH(0,0)),
                    F10(Y.SH(1,0)),
                    F11(Y.SH(1,1));
        complex<double> f00, f11;
        Axis< double > invgamma(pr);
        for (size_t ip(0); ip < pr.dim(); ++ip) invgamma(ip) = 1.0 / sqrt(1.0+pr(ip)*pr(ip)); 
        // ---------  Jx  ------------------------------------ 
        if (Inputdata::IN().list().o_Jx) {
			Matrix2D<float> Jx(szx,szy);  

			//f0 = Y.SH(1,0); using reference instead
			for (int iy(0); iy < szy; ++iy){
				for (int ix(0); ix < szx; ++ix){

					 Jx(ix,iy)  = F10(0,ix+Nbc,iy+Nbc).real() * pow(pr(0),3)*invgamma(0);
					 Jx(ix,iy) += F10(F10.nump()-1,ix+Nbc,iy+Nbc).real() *
									  pow(pr(pr.dim()-1),3)*invgamma(pr.dim()-1);
					 Jx(ix,iy) *= 0.5;
					 for (size_t ip(1); ip < F10.nump()-1; ++ip) { 
						 Jx(ix,iy)+= pow(pr(ip),3)*invgamma(ip)*F10(ip,ix+Nbc,iy+Nbc).real();
					 }
			...
			Jx *= (-4.0 * M_PI / 3.0) * pr.dx();
			//not divided by the density

            string fname("OUTPUT/FLD/JX/Jx");
            parallel_mom(step, Jx, fname);

		}

        // ---------  Jy+Jz  ------------------------------------ 
        if (Inputdata::IN().list().o_Jy || Inputdata::IN().list().o_Jz) {
			Matrix2D<float> Jy(szx,szy); 
			Matrix2D<float> Jz(szx,szy); 

			//f0 = Y.SH(1,1); using reference instead
			for (int iy(0); iy < szy; ++iy){
				for (int ix(0); ix < szx; ++ix){

					Jy(ix,iy)  = F11(0,ix+Nbc,iy+Nbc).real() * pow(pr(0),3)*invgamma(0);
					Jy(ix,iy) += F11(F11.nump()-1,ix+Nbc,iy+Nbc).real() * 
									 pow(pr(pr.dim()-1),3)*invgamma(pr.dim()-1);
					Jy(ix,iy) *= 0.5;
					//Jz
					Jz(ix,iy)  = F11(0,ix+Nbc,iy+Nbc).imag() * pow(pr(0),3)*invgamma(0);
					Jz(ix,iy) += F11(F11.nump()-1,ix+Nbc,iy+Nbc).imag() * 
									 pow(pr(pr.dim()-1),3)*invgamma(pr.dim()-1);
					Jz(ix,iy) *= 0.5;

					for (size_t ip(1); ip < F11.nump()-1; ++ip){ 
						Jy(ix,iy)+= pow(pr(ip),3)*invgamma(ip)*F11(ip,ix+Nbc,iy+Nbc).real();
						Jz(ix,iy)+= pow(pr(ip),3)*invgamma(ip)*F11(ip,ix+Nbc,iy+Nbc).imag();
					}

					f11  =  F11(1,ix+Nbc,iy+Nbc);
			...
			if (Inputdata::IN().list().o_Jy) {
				Jy *= (-8.0 * M_PI / 3.0) * pr.dx();
				string fname("OUTPUT/FLD/JY/Jy");
				parallel_mom(step, Jy, fname);
			}
			if (Inputdata::IN().list().o_Jz) {
				Jz *=(8.0 * M_PI / 3.0) * pr.dx();
				string fname("OUTPUT/FLD/JZ/Jz");
				parallel_mom(step, Jz, fname);
			}
		}
...
//--------------------------------------------------------------
        ...
        // ---------  Density  ---------                                                        //WHY- P18,density, gamma == 1 
        moment = 0.0;
        ...
        for (int iy(0); iy < szy; ++iy){
         ...
         moment *= 4.0 * M_PI * pr.dx();                                                        //WHY? dp x p.^3 ? error
         if (Inputdata::IN().list().o_x1x2) parallel_mom(step, moment, fname); 
        // ---------------------------- 
              
        // ---------  Fs(p=const,x,y)  ---------
         if (Inputdata::IN().list().o_fsp) {
			 //large p
             fname = "OUTPUT/DISTR/Fs/Fl0";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f0(199,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);

			 SHarmonic f1(Y.SH(1,0));
             fname = "OUTPUT/DISTR/Fs/Fl1";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f1(199,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);

			 SHarmonic f2(Y.SH(2,0));
             fname = "OUTPUT/DISTR/Fs/Fl2";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f2(199,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);
			 //small p
             fname = "OUTPUT/DISTR/Fs/Fs0";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f0(1,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);

             fname = "OUTPUT/DISTR/Fs/Fs1";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f1(1,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);

             fname = "OUTPUT/DISTR/Fs/Fs2";
             for (int iy(0); iy < szy; ++iy){
                 for (int ix(0); ix < szx; ++ix){
					 moment(ix,iy) = f2(1,ix+Nbc,iy+Nbc).real();
                 }
             }
             parallel_mom(step, moment, fname);
		 }
...
//
//--------------------------------------------------------------
    void Parallel_Output::parallel_pdistr_1p(size_t step, Stat& Y) {
        using Inputdata::IN; 
		// only care l=0,1,2,3,4
		size_t l_num = 24;
		int i=0;
		// 
        Matrix2D<float> Ps(pr.dim(), l_num);

		for(int l=0;l<5;l++){
			for(int m=0;m<=(l<=2?l:2);m++){
				SHarmonic&  F(Y.SH(l,m));
				for(int ip(0); ip<pr.dim();ip++){
					Ps(ip,i)   = F(ip,Nbc, Nbc).real();
					Ps(ip,i+1) = F(ip,Nbc, Nbc).imag();
				}
				//cout<<"loop:"<<l<<" "<<m<<" "<<i<<endl;
				i +=2;
			}
		}
		
		//cout<<"now pdistr"<<endl;
		//outputs
        //string fname("OUTPUT/DISTR/Pmulti/");
        MPI_Status status;

        size_t st(0), bi(0);
        int msg_sz(l_num*pr.dim());
        float* p1x1buf = new float[msg_sz];
        Matrix2D<float> Mp1x1glob(pr.dim(), l_num*NODES() );

        for(int i(0); i < Ps.dim(); ++i) p1x1buf[i] = Ps(i);
        ...
        if (NODES() > 1) { 
           if (RANK()!=0) {
              MPI_Send(p1x1buf, msg_sz, MPI_FLOAT, 0, RANK(), MPI_COMM_WORLD);
               ...
               // Fill data for rank = 0 
               bi = 0;
               for(GSIter_F it(Mp1x1glob.SubMatrix2D(0, pr.dim(),l_num)); it != it.end(); ++it) 
                   *it = p1x1buf[bi++];
               // Fill data for rank > 0 
               for (int rr(1); rr < NODES(); ++rr){
                   MPI_Recv(p1x1buf, msg_sz, MPI_FLOAT, rr, rr, MPI_COMM_WORLD, &status);
                   st = (rr % NODESX())*l_num*pr.dim();
                   bi = 0;
                   for(GSIter_F it(Mp1x1glob.SubMatrix2D(st, pr.dim(),l_num)); it != it.end(); ++it) 
                       *it = p1x1buf[bi++]; 
               }
            }
         }
		 ...
		 //cout<<"now export"<<endl;
         if (RANK() == 0)  {
             exportdata.Export_ps(step, Mp1x1glob);
         }
...
//--------------------------------------------------------------
        ...
        IN().inp().x(0) = IN().inp().xglob(rankx*(IN().inp().x.dim()-2*IN().list().RKLevel))    //WHY? question about RK
...
//--------------------------------------------------------------
    void Parallel_Environment::Neighbor_Communications(Stat& Y){                                    //WHY- the difference method doesn't need to communicate 4 corners
//--------------------------------------------------------------
//  Information exchange between neighbors 
//--------------------------------------------------------------
        int moduloX(RANKX()%2); 
        ...
        if (NODESX() > 1) {
            ...
            if (moduloX==0){
                ...
                if ((RANKX() != 0) || (BNDX()==0)){                                                 //WHY- bndx==0 periodic, =1 mirror
                    ...
                    if (BNDX()==1) {
                        ...
                        //X_Data.smooth_bound_Xleft(Y);
                        X_Data.contin_bound_Xleft(Y);
                    }
                    else {
                        cout<<"Invalid Boundary.\n";
                    }
            ...
            //odd nodes 
                ...
                if ((RANKX()!=(NODESX()-1)) || (BNDX()==0)){
                     X_Data.Send_right_X(Y,RNx);              //   (Send)  1 --> 2 
                     X_Data.Recv_from_right_X(Y,RNx);         // (Receive) 1 <-- 2
                }
                else {
                    if (BNDX()==1) {
                        ...
                        //X_Data.smooth_bound_Xright(Y);
                        X_Data.contin_bound_Xright(Y);
                        //X_Data.mirror_bound_Xright(Y);        // Update node "N-1" in the x direction
                    }
                    else {
                        cout<<"Invalid Boundary.\n";
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
         ...
         int moduloY(RANKY()%2);
         int RNy((RANK()+NODESX())%NODES()),                  // This is the right neighbor //WHY- or lower 
             LNy((RANK()-NODESX()+NODES())%NODES());          // This is the left  neighbor //WHY- or upper 
         ...
         if (NODESY() > 1) {
            ...
            //odd nodes 
                ...
                if ((RANKY()!=(NODESY()-1)) || (BNDY()==0)){
                     X_Data.Send_right_Y(Y,RNy);                //   (Send)  1 --> 2
                     X_Data.Recv_from_right_Y(Y,RNy);           // (Receive) 1 <-- 2
                }
                else {
                    if (BNDY()==1) {
...
//--------------------------------------------------------------
    void Parallel_Environment::Update_XRedge(Stat& Y){ 
		//works for nodesx >1 & bound_x is mirror
        int moduloX(RANKX()%2); 
		EMF2D& EF(Y.EMF());
		int yid, xid;
        size_t Nx(Y.SH(0,0).numx()), Ny(Y.SH(0,0).numy()), Nbc(Inputdata::IN().list().RKLevel);
        int szy = Ny-2*Nbc+2;
		    xid = Nx-Nbc;
        ...
        if (NODESX() > 1) {
            if (moduloX !=0 && BNDX()==2){
				for (int iy(0); iy < szy; ++iy){
					yid = iy+Nbc-1;
					EF.Ex()(xid,yid) = EF.Ex()(xid-1,yid);
					EF.By()(xid,yid) = EF.By()(xid-1,yid);
					EF.Bz()(xid,yid) = EF.Bz()(xid-1,yid);
					EF.Ex()(xid+1,yid) = EF.Ex()(xid-1,yid);
					EF.By()(xid+1,yid) = EF.By()(xid-1,yid);
					EF.Bz()(xid+1,yid) = EF.Bz()(xid-1,yid);
				}
			}
		}
	}
...
//--------------------------------------------------------------
        ...
        if (IN().list().o_Jx || IN().list().o_Jy || IN().list().o_Jz){
            //Current_xyz JN;
            //Ampere();
            //Out_Data.parallel_Jamp(step,Y);
			//VxB
            //Out_Data.parallel_Jv(step,Y);
        }
		...
		//cout<<"now out_data"<<endl;
        if ( IN().list().o_pmulti >0)  {
            Out_Data.parallel_pdistr_1p(step,Y);
        }
    }
//--------------------------------------------------------------
