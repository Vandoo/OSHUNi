...
//
//   2. class EMF2D :
//       This is the collection of the Electromagnetic fields.  //totally 6
// 
//   3. class SHarmonic :
...
//  Access
//--------------------------------------------------------------
    GSlice_iter< complex<double> >  Field2D::TrimGuards(size_t cN){     //submatrix[Numx-2cN]^2
...
//--------------------------------------------------------------
    ...
    Field2D& Field2D::Im(){
        for (int i(0); i < numx()*numy(); ++i) (*sh)(i) -= (*sh)(i).real(); //WHY? (-=...real) not (=...imag)?
        return *this;
    }
//--------------------------------------------------------------
    Field2D& Field2D::Dx(){
...
//  Copy assignment operator
    ...
    SHarmonic& SHarmonic::operator=(const SHarmonic& other){
        if (this != &other) {   //self-assignment
           *sh = other.matrix();                                //WHY- matrix() isconst *sh 
        }
        return *this;
    }
//  *= 
...
//--------------------------------------------------------------
    SHarmonic& SHarmonic::mxy_matrix(Matrix2D< complex<double> >& mMatrix){ //WHY? what is mxy?
        int st(0), nxt(nump());
...
//--------------------------------------------------------------
        ...
        GSlice_iter< complex<double> > it1(p0(nump()-2)), it2(p0(nump()-1)); //WHY- p0 defined in state.h 
        for(int i=0; i< numx()*numy(); ++i){                    //WHY- deriv at np-1 
            ma(i)  = *it1; ma(i) -= *it2;
            ++it1; ++it2;
        }
...
//  *=
    Stat& Stat::operator*=(const Stat& other){                  //WHY? when do you need to evolve Dfunc&EMF at the same time?
        *ff *= other.DF();
        *em *= other.EMF();
        return *this;
    }
