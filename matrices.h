...
//--------------------------------------------------------------
        valarray<T>* v;
        slice s;                                                //std::slice
        size_t curr;     // index of current element            //WHY- seems slice.start is enough
    ...
    public:
        ...
        Slice_iter  operator++(int) {Slice_iter t = *this; ++curr; return t;} //postfix     //WHY? whats diff above
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// The Constant Slice Iterator is not currently in use          //the Cslice_iter
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//--------------------------------------------------------------
    ...
    public:
        ...
        GSlice_iter end() const {
            GSlice_iter t = *this;
            t.curr = gsizes[0];    //WHY! check the constructor, the same
            return t;
        }
...
//--------------------------------------------------------------
    ...
    bool operator==(const GSlice_iter<T>& p, const GSlice_iter<T>& q){
        ...
        if (p.curr==q.curr && p.gs.start() == q.gs.start()) 
            while (  (p.gs.stride()[count] == p.gs.stride()[count])         //WHY?stride()[]
...
//      Access
        ...
        Slice_iter<T>  d2c(size_t b, size_t e);     //(e-b+1)*d1
...
//      Operators
        ...
        Matrix2D& operator*=(const T& d);
        Matrix2D& operator*=(const Matrix2D& vmulti);           //like the above, scalar multi
        Matrix2D& operator+=(const T& d);
...
//--------------------------------------------------------------
// Central difference   //WHY? Dd1 & Dd2 s' last line are different
//--------------------------------------------------------------
...
//--------------------------------------------------------------
    ...
    public:
//      Constructor (for symmetric axis)                        //*2 -> [-2,0,2],etc
        Axis(size_t numx, T xmax){
            ax = new  valarray<T>(numx);
            for (int i = 0; i < numx; ++i)                      //WHY?this construtor is wasted in parallel_enironment:: IN().inp().x(...)?
                (*ax)[i] = i;
            (*ax) *= xmax * (2.0/(static_cast<T>(numx-1)));
        (*ax) -= xmax;
        }
//      Constructor (for nonsymmetric axis)                     //[2,3,4],etc
